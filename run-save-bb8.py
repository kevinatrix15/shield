...
def runSolve(nx, ny, rad, case):
  cmd = [str(SAVE_BB8), str(ny), str(nx), str(rad), str(case)]
  try:
...
def calcAvgTime(n, nx, ny, rad, case):
  times = np.zeros(n)
  for i in range(n):
    t0 = tt.time()
    runSolve(nx, ny, rad, case)
    t1 = tt.time()
    times[i] = t1 - t0
  return np.mean(times)

def timeStats(case):
  dims = np.array([100, 500, 1000, 2500, 3500, 6000])
  avgTimes = np.zeros(len(dims))
  for i in range(len(dims)):
    avgTimes[i] = calcAvgTime(10, dims[i], dims[i], 5, case)
    print('dim: ', dims[i])
    print('avgTime: ', avgTimes[i])

  return dims, avgTimes
...
def main():
  ...
  for c in range(1,6):
    dims, caseAvgs = timeStats(c)
    plt.plot(dims, caseAvgs, label="Case" + str(c))

  plt.xlabel("Dimension [cells]")
  plt.ylabel("Avg Time [sec]")
  plt.legend()
  fig.savefig('case-comps.png')
  #fig.savefig('time-stats' + str(case) + '.png')
  """
